...
async def list_active_reminders(user_id: int, page: int = 1, per_page: int = 5, category: str | None = None, priority: str | None = None) -> tuple[list[dict], int]:
    ...
    where = [BASE_SCOPE, "status IN ('active','snoozed')"]
...
async def stats(user_id: int) -> dict:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        total = (await cur.fetchone())[0]
        cur = await db.execute(
            f"SELECT status, COUNT(*) FROM reminders WHERE {BASE_SCOPE} GROUP BY status",
            (user_id, user_id),
        )
        by_status = {row[0]: row[1] for row in await cur.fetchall()}
        cur = await db.execute(
            f"SELECT category, COUNT(*) FROM reminders WHERE {BASE_SCOPE} GROUP BY category",
            (user_id, user_id),
        )
        by_category = {row[0]: row[1] for row in await cur.fetchall()}
        cur = await db.execute(
            f"SELECT priority, COUNT(*) FROM reminders WHERE {BASE_SCOPE} GROUP BY priority",
            (user_id, user_id),
        )
...
async def list_all_reminders(user_id: int) -> list[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            f"SELECT * FROM reminders WHERE {BASE_SCOPE} ORDER BY scheduled_at_utc ASC",
            (user_id, user_id),
        )
        return [dict(r) for r in await cur.fetchall()]
