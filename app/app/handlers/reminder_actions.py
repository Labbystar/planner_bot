...
async def _notify_owner(bot: Bot, reminder: dict | None, actor: dict | None, status_label: str, extra: str | None = None) -> None:
    if not reminder or not actor:
        return
    ...
    if not owner:
        return
    actor_label = ('@' + actor['username']) if actor.get('username') else actor.get('full_name') or f"ID {actor['user_id']}"
    text = (
        f"📣 Обновление по задаче\n\n"
        f"📌 {reminder['text']}\n"
        f"👤 {actor_label}\n"
        f"{status_label}"
    )
...
async def accept_task(callback: CallbackQuery, bot: Bot) -> None:
    ...
    if not _can_act(reminder, viewer):
        ...
        return
    await _notify_owner(bot, reminder, viewer, "👁 Статус: Принял")
    await callback.answer("Задача принята")


@router.callback_query(F.data.startswith("done:"))
async def done(callback: CallbackQuery, bot: Bot) -> None:
    reminder_id = int(callback.data.split(":", 1)[1])
    reminder = await get_reminder(reminder_id)
    viewer = await get_user(callback.from_user.id)
    if not _can_act(reminder, viewer):
    ...
    await _notify_owner(bot, updated, viewer, "✅ Статус: Выполнено")
...
async def do_snooze(callback: CallbackQuery, bot: Bot) -> None:
    ...
    await _notify_owner(bot, updated, viewer, "⏰ Статус: Отложено", f"🕓 На {minutes} мин")
...
async def comment_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.answer()
    await callback.message.answer("Напиши комментарий по задаче. Он придёт постановщику.")


@router.message(EditReminder.comment)
async def comment_apply(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    if not reminder_id or not _can_act(reminder, viewer):
        await state.clear()
    ...
    updated = await get_reminder(reminder_id)
    await _notify_owner(bot, updated, viewer, "💬 Новый комментарий", f"📝 {comment}")
    await state.clear()
    await message.answer("✅ Комментарий отправлен постановщику.")
